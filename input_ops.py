...
def create_input_ops(dataset,
    ...
    with tf.device("/cpu:0"), tf.name_scope(scope):
        ...
        m, l = dataset.get_data(data_id[0])

        def load_fn(id):
            # image [h, w, c]
            # label [v]

            # below for testing (generate txt)
            #print('input ops id', id)
            # end for testing (generate txt)

            image, label = dataset.get_data(id.decode('utf-8'))
...
def create_input_ops_resnet(dataset,
                            batch_size,
                            num_threads=16,           # for creating batches
                            is_training=False,
                            data_id=None,
    ...
    with tf.device("/cpu:0"), tf.name_scope(scope):
        ...
        m, l = dataset.get_data_resnet(data_id[0])

        def load_fn_resnet(id):
            image, label = dataset.get_data_resnet(id.decode('utf-8'))
            return (id, image.astype(np.float32), label.astype(np.float32))

        input_ops['id'], input_ops['image'], input_ops['label'] = tf.py_func(
            load_fn_resnet, inp=[input_ops['id']],
            Tout=[tf.string, tf.float32, tf.float32],
            name='func_hp'
        )
