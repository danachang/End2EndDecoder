...
def create_default_splits(path, is_train=True):
    train_ids, train_labels = get_video_path_and_label(osp.join(path, 'train'))
    val_ids, val_labels = get_video_path_and_label(osp.join(path, 'val'))
    test_ids, test_labels = get_video_path_and_label(osp.join(path, 'test'))
...
def get_video_path_and_label(path):
    ids = []
    labels = {}
