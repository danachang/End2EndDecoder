...
from model_neuron import Model
import dataset_neuron
...
def argparser(is_train=True):
    ...
    dataset_train, dataset_val, dataset_test = \
        dataset_neuron.create_default_splits(config.dataset_path)

    activity, label = dataset_train.get_data(dataset_train.ids[0])
    config.l = len(activity)
