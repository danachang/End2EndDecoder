...
from model_video_conv3d import Model
import dataset_video_conv3d
...
def argparser(is_train=True):
    ...
    dataset_train, dataset_val, dataset_test = \
        dataset_video_conv3d.create_default_splits(config.dataset_path)

    video, label = dataset_train.get_data(dataset_train.ids[0])
    config.d = video.shape[0]
    config.h = video.shape[1]
    config.w = video.shape[2]
    config.c = video.shape[3]
