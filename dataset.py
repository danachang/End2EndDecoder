...
class Dataset(object):
    ...
    def get_data(self, id):
        image = imread(id)/255.*2-1
        if not len(image.shape) > 2:
            image = np.expand_dims(image, -1)
        label = self._labels[id]
        return image, label

    def get_data_resnet(self, id):
        image = imread(id)
        if not len(image.shape) > 2:
            image = np.expand_dims(image, -1)
        label = self._labels[id]
...
def create_default_splits(path, is_train=True):
    train_ids, train_labels = get_image_path_and_label(osp.join(path, 'train'))
    val_ids, val_labels = get_image_path_and_label(osp.join(path, 'val'))
    test_ids, test_labels = get_image_path_and_label(osp.join(path, 'test'))
...
def get_image_path_and_label(path):
    ids = []
    labels = {}
