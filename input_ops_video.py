...
def create_input_ops(dataset,
    ...
    with tf.device("/cpu:0"), tf.name_scope(scope):
        ...
        v, l = dataset.get_data(data_id[0])
        ...
        def load_fn(id):
            ...
            video, label = dataset.get_data(id.decode('utf-8'))
            return (id, video.astype(np.float32), label.astype(np.float32))

        input_ops['id'], input_ops['video'], input_ops['label'] = tf.py_func(
        ...
        input_ops['video'].set_shape(list(v.shape))
